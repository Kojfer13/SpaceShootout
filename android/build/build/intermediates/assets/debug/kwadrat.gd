...
func skin(id, ifgolden):
	...
	match id:
		...
		2:
			if ifgolden:
				$Sprite2D.texture = preload("res://Skins/Golden/kosmita_diamentowy.png")
			else:
				$Sprite2D.texture = preload("res://Skins/Golden/kosmita_midas.png")
			$Sprite2D.scale.x = 1
			$Sprite2D.scale.y = 1
			$Sprite2D.rotation = 0
		3:
			if ifgolden:
				...
				$Sprite2D.texture = preload("res://statek.png")
...
func setskin(val):
	match val:
		...
		_:
			$Sprite2D.set_texture = preload("res://Skins/kosmita.png")
func poza_ekranem():
	poza.emit()
	queue_free()
